...
/// that must be done by wrapping it before calling [crawlSync].
List<V> crawlSync<K extends Object, V>(
  Iterable<K> roots,
  V Function(K) readNode,
  Iterable<K> Function(K, V) edges,
) {
  final crawl = _CrawlSync(roots, readNode, edges)..run();
  return crawl.result;
}

class _CrawlSync<K, V> {
  final result = <V>[];

  final V Function(K) readNode;
  final Iterable<K> Function(K, V) edges;
  final Iterable<K> roots;

  final _seen = HashSet<K>();

  _CrawlSync(this.roots, this.readNode, this.edges);
  ...
  /// edges.
  void _crawlFrom(K key) {
    final value = readNode(key);
    result.add(value);
    final next = edges(key, value);
    next.forEach(_visit);
  }
